...
func UserGeneralLogin(c yee.Context) (err error) {
	...
	if err := model.DB().Where("username = ?", u.Username).First(&account).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		...
		if e := lib.DjangoCheckPassword(&account, u.Password); e {
			...
			model.DB().Select("secret,is_mfa").Model(model.CoreAccount{}).Where("username =?", u.Username).First(&account)

			if account.IsMfa == 1 && account.Secret != "" {
				dataStore["is_mfa"] = true
			}
...
func UserMfaLogin(c yee.Context) (err error) {
	u := new(loginForm)
	if err = c.Bind(u); err != nil {
		c.Logger().Error(err.Error())
		return c.JSON(http.StatusOK, common.ERR_REQ_BIND)
	...
	if err := model.DB().Where("username = ?", u.Username).First(&account).Error; !errors.Is(err, gorm.ErrRecordNotFound) {
		...
		valid := totp.Validate(strconv.Itoa(int(u.Passcode)), account.Secret)
		if valid {
			token, tokenErr := lib.JwtAuth(lib.Token{
				Username: u.Username,
				RealName: account.RealName,
