...
func MessagePush(workid string, t uint, reject string) {
	...
	model.DefaultLogger.Debugf("GloOther.Query: %v", model.GloOther.Query)
	...
	if model.GloOther.Query && t > 6 {
		...
		if t == 7 {
			...
			wechat = chatMsgTplHandler("已提交", op)
		...
		if t == 8 {
			...
			wechat = chatMsgTplHandler("已同意", op)
		...
		if t == 1 {
			...
			wechat = chatMsgTplHandler("已执行", o)
		...
		if t == 2 {
			...
			wechat = chatMsgTplHandler("已提交", o)
		...
		if t == 4 {
			...
			wechat = chatMsgTplHandler("执行失败", o)
		...
		if t == 5 {
			...
			wechat = chatMsgTplHandler("已转交至下一操作人", o)
		...
		if t == 6 {
			...
			wechat = chatMsgTplHandler("已撤销", o)
	...
	if model.GloMessage.Wechat {
		if model.GloMessage.ChatHook != "" {
			go SendWechatMsg(s.Message, wechat)
		}
	}
}
